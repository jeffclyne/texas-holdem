...
const dealHand = () => {
  ...
  // Blinds
  activePlayers[(dealerIndex + 1) % players.length].currentBet = smallBlind;
  activePlayers[(dealerIndex + 2) % players.length].currentBet = smallBlind * 2;
  pot += smallBlind * 3;
  currentBet = smallBlind * 2;
  actIndex = (dealerIndex + 3) % players.length;
